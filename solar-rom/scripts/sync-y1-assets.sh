...
mkdir -p "$DST"
for f in switch-to-stock.sh switch-to-rockbox.sh sync-rockbox-libs.sh sync-y1-keymap.sh disable-rockbox-for-solar.sh solar-usb-recovery-agent.sh solar-deferred-init.sh Y1-Rockbox.kl; do
    cp "$SRC/$f" "$DST/$f"
done
